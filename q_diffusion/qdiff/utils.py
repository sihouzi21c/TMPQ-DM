...
from qdiff.quant_layer import QuantModule, UniformAffineQuantizer,QuantModule_one,QuantModule_darts,QuantModule_inference
from qdiff.quant_block import BaseQuantBlock,BaseQuantBlock_one,BaseQuantBlock_darts,QuantAttnBlock_darts
...
def get_train_samples_txt(args, sample_data, custom_steps=None):
    ...
    if num_st == 1:
        ...
        lst_chosen=[random.choice(timesteps) for i in range(num_samples)]
        xs_lst = [sample_data['xs'][lst_chosen[i]][i].unsqueeze(0).clone()for i in range(num_samples)]
        ts_lst = [sample_data['ts'][lst_chosen[i]][i].unsqueeze(0).clone()for i in range(num_samples)]
        if args.cond:
            xs_lst += xs_lst
            ts_lst += ts_lst
            conds_lst = [sample_data['cs'][lst_chosen[i]][i].unsqueeze(0).clone()for i in range(num_samples)] + [sample_data['ucs'][lst_chosen[i]][i].unsqueeze(0).clone()for i in range(num_samples)]
...
def get_train_samples(args, sample_data, custom_steps=None):
    ...
    return xs, ts
    '''
    if args.datasets_to_study == 'cifar':
        custom_steps = args.custom_steps if custom_steps is None else custom_steps
        if num_st == 1:
            xs = sample_data[:num_samples]
            ts = (torch.ones(num_samples) * 800)
            ...
            if args.cond:
                ...
                return xs, ts, conds
        return xs, ts
    elif args.datasets_to_study == 'ImageNet64':
        timesteps_seq = (np.linspace(0, np.sqrt(800), 100) **2)
        timesteps_seq = [int(s) for s in list(timesteps_seq)]
        def trans(x):#timestep [-1,1000]-->index [1000,0]
            return 999-x
        nsteps = 100
        assert num_st != 1
        timesteps = list(range(0, nsteps, nsteps//num_st))
        timesteps_chosen = [timesteps_seq[mmm] for mmm in timesteps] 
        ind = [trans(xx)for xx in timesteps_chosen]
        xs_lst = [sample_data['xs'][xx][:num_samples] for xx in ind]
        ts_lst = [torch.ones(num_samples) * xx for xx in timesteps_chosen]
        conds = [torch.clone(sample_data['cs'][:num_samples]) for xx in timesteps_chosen]
        xs = torch.cat(xs_lst, dim = 0)
        ts = torch.cat(ts_lst, dim = 0)
...
def Parameterize_weight(qnn):
    for m in qnn.model.modules():
        if isinstance(m, AdaRoundQuantizer):
            m.zero_point = nn.Parameter(m.zero_point)
            m.delta = nn.Parameter(m.delta)

def Parameterize_act(qnn):
    for m in qnn.model.modules():
        if isinstance(m, AdaRoundQuantizer):
            m.zero_point = nn.Parameter(m.zero_point)
            m.delta = nn.Parameter(m.delta)
        elif isinstance(m, UniformAffineQuantizer):#this is for weight_quantization
            if m.zero_point is not None:
                if not torch.is_tensor(m.zero_point):
                    m.zero_point = nn.Parameter(torch.tensor(float(m.zero_point)))
                else:
                    m.zero_point = nn.Parameter(m.zero_point)
#Parameterize--> load --> UnParameterize
def UnParameterize_weight(qnn):
    for m in qnn.model.modules():
        if isinstance(m, AdaRoundQuantizer):
...
def UnParameterize_act(qnn):
    for m in qnn.model.modules():
        if isinstance(m, AdaRoundQuantizer):
            ...
            delattr(m, "delta")
            m.delta = delta_data
        elif isinstance(m, UniformAffineQuantizer):#this is for act_quantizer
            if m.zero_point is not None:
...
def convert_adaround_one(model):
    for name, module in model.named_children():
        if isinstance(module, QuantModule_one):
            if module.ignore_reconstruction is True:
                ...
                if module.split != 0:#if hasattr(sub_module,'weight_quantizer_0'):#if sub_module.split != 0:
                            # print(f"split {name}")
                    for i in range(len(module.weight_quantizer_list)):
                        module.weight_quantizer_list[i] = AdaRoundQuantizer(uaq=module.weight_quantizer_list[i], round_mode='learned_hard_sigmoid',
                                                                        weight_tensor=module.org_weight.data[:, :module.split, ...])
                        module.weight_quantizer_0_list[i] = AdaRoundQuantizer(uaq=module.weight_quantizer_0_list[i], round_mode='learned_hard_sigmoid',
                                                                        weight_tensor=module.org_weight.data[:, module.split:, ...])
                else:
                    for i in range(len(module.weight_quantizer_list)):
                        module.weight_quantizer_list[i] = AdaRoundQuantizer(uaq=module.weight_quantizer_list[i], round_mode='learned_hard_sigmoid',
            ...
            if module.ignore_reconstruction is True:
                ...
                for name, sub_module in module.named_modules():
                    ###也许需要修改到ignore版本？
                    if isinstance(sub_module, QuantModule_one):
                        if sub_module.split != 0:#if hasattr(sub_module,'weight_quantizer_0'):#if sub_module.split != 0:
                            # print(f"split {name}")
                            for i in range(len(sub_module.weight_quantizer_list)):
                                sub_module.weight_quantizer_list[i] = AdaRoundQuantizer(uaq=sub_module.weight_quantizer_list[i], round_mode='learned_hard_sigmoid',
                                                                    weight_tensor=sub_module.org_weight.data[:, :sub_module.split, ...])
                                sub_module.weight_quantizer_0_list[i] = AdaRoundQuantizer(uaq=sub_module.weight_quantizer_0_list[i], round_mode='learned_hard_sigmoid',
                                                                    weight_tensor=sub_module.org_weight.data[:, sub_module.split:, ...])
                        else:
                            for i in range(len(sub_module.weight_quantizer_list)):
                                sub_module.weight_quantizer_list[i] = AdaRoundQuantizer(uaq=sub_module.weight_quantizer_list[i], round_mode='learned_hard_sigmoid',
            ...
            convert_adaround_one(module)

def resume_cali_model_one(qnn, ckpt_path, cali_data,len_weight_list,quant_act=False, act_quant_mode='qdiff', cond=False):
    def forward_model(model,cali_xs,cali_ts,cali_cs=None):
        for i in range(len_weight_list):
            model.set_quant_idx(i)
            if cali_cs is None:
                _ = model(cali_xs, cali_ts)
            else:
                _ = model(cali_xs, cali_ts,cali_cs)
    logger.info("Loading quantized model checkpoint")
    ...
    if not cond:
        cali_xs, cali_ts = cali_data
        forward_model(qnn,cali_xs[:1].cuda(),cali_ts[:1].cuda())
    else:
        cali_xs, cali_ts, cali_cs = cali_data
        forward_model(qnn,cali_xs[:1].cuda(),cali_ts[:1].cuda(),cali_cs[:1].cuda())
    # change weight quantizer from uniform to adaround
    convert_adaround_one(qnn)
    for m in qnn.model.modules():
        if isinstance(m, AdaRoundQuantizer):
            m.zero_point = nn.Parameter(m.zero_point)
            m.delta = nn.Parameter(m.delta)
    ...
    if quant_act:       
        logger.info("Initializing act quantization parameters")
        qnn.set_quant_state(True, True)
        if not cond:
            forward_model(qnn,cali_xs[:1].cuda(),cali_ts[:1].cuda())
        else:
            forward_model(qnn,cali_xs[:1].cuda(),cali_ts[:1].cuda(),cali_cs[:1].cuda())
        logger.info("Loading quantized model checkpoint again")
    ...
    # change weight quantizer from uniform to adaround
    convert_adaround_inference(qnn)
    for m in qnn.model.modules():
        if isinstance(m, AdaRoundQuantizer):
            m.zero_point = nn.Parameter(m.zero_point)
            m.delta = nn.Parameter(m.delta)
            ...
            m.inited=False
            if m.delta is None:#because there is QuantBlock
                pass
            else:
                # pdb.set_trace()
                delta=m.delta.data
