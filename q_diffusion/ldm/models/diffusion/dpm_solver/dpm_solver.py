...
class DPM_Solver:
    ...
    def sample_timesteps(self, x, steps, t_start=None, t_end=None, order=3, skip_type='time_uniform',
        method='singlestep', lower_order_final=True, denoise_to_zero=False, solver_type='dpm_solver',
        atol=0.0078, rtol=0.05,
    ): 
        t_0 = 1. / self.noise_schedule.total_N if t_end is None else t_end
        ...
        if method == 'adaptive':
            ...
            with torch.no_grad():
                x = self.dpm_solver_adaptive(x, order=order, t_T=t_T, t_0=t_0, atol=atol, rtol=rtol, solver_type=solver_type).cuda().to(torch.float32)
        elif method == 'multistep':#in this way?
            assert len(steps)-1 >= order
            timesteps = np.array(sorted(steps, reverse=True))

            timesteps=torch.from_numpy(timesteps)
            with torch.no_grad():
                vec_t = timesteps[0].expand((x.shape[0])).cuda()
                model_prev_list = [self.model_fn(x, vec_t)]
                t_prev_list = [vec_t]
                # Init the first `order` values by lower order multistep DPM-Solver.
                steps_now=len(timesteps)-1
                for init_order in range(1, order):
                    vec_t = timesteps[init_order].expand(x.shape[0]).cuda().to(torch.float32)
                    x = self.multistep_dpm_solver_update(x, model_prev_list, t_prev_list, vec_t, init_order, solver_type=solver_type).to(torch.float32)
                    model_prev_list.append(self.model_fn(x, vec_t))
                    t_prev_list.append(vec_t)
                # Compute the remaining values by `order`-th order multistep DPM-Solver.
                for step in range(order, steps_now+1):#steps_now==len(steps)-1
                    vec_t = timesteps[step].expand(x.shape[0]).cuda()
                    if lower_order_final and steps_now < 15:#len(steps) < 15:
                        # step_order = min(order, len(steps) + 1 - step)
                        step_order = min(order, steps_now + 1 - step)
                    else:
                        step_order = order
                    x = self.multistep_dpm_solver_update(x, model_prev_list, t_prev_list, vec_t, step_order, solver_type=solver_type).to(torch.float32)
                    for i in range(order - 1):
                        t_prev_list[i] = t_prev_list[i + 1]
                        model_prev_list[i] = model_prev_list[i + 1]
                    t_prev_list[-1] = vec_t
        ...
        if denoise_to_zero:
            # x = self.denoise_to_zero_fn(x, torch.ones((x.shape[0],)).to(device) * t_0)
            x = self.denoise_to_zero_fn(x, torch.ones((x.shape[0],)).cuda() * t_0)
        return x
