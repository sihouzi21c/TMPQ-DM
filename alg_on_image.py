...
from qdiff.utils import resume_cali_model_one
...
from utils.cal_fid import _compute_statistics_of_path,InceptionV3, calculate_activation_statistics,calculate_frechet_distance
...
class LatentDiffusion(object):
    def __init__(self, args, config,device=None):
        ...
        self.model=self.get_model(self.weight_list,self.act_list,'cifar5678',cali_ckpt=args.ckpt_pre)
    ...
    def get_model(self,weight_list,act_list,ckptsavename,cali_ckpt='no'):
        ...
        model = load_model_from_config(config, self.args.model_ckpt)
        ...
        logger.info('FP model params size:{} MB'.format(size_model / 8e6))
        from quant_dataset import DiffusionInputDataset
        from torch.utils.data import DataLoader
        dataset = DiffusionInputDataset(self.args.cali_data_path)
        ...
        if self.args.ptq:
            if self.args.quant_mode == 'qdiff':
                ...
                cali_xs, cali_ts, cali_cs = get_train_samples(data_loader, num_samples=1024)
                cali_data = (cali_xs, cali_ts, cali_cs)
                gc.collect()
                if self.args.resume:
                    resume_cali_model_one(qnn, cali_ckpt, cali_data, len(self.weight_list), args.quant_act, "qdiff", cond=True)
                else:
                    ...
                    if self.args.resume_w:
                        ...
                        self.forward_model(qnn,cali_xs[:8].cuda(),cali_ts[:8].cuda(),cali_cs[:8].cuda())
                        logger.info("Initializing has done!")
                    kwargs = dict(cali_data=cali_data, cond=True, batch_size=self.args.cali_batch_size, 
                    ...
                    if self.args.quant_act:
                        ...
                        kwargs = dict(
                            cali_data=cali_data, cond = True, iters=self.args.cali_iters_a, act_quant=True, 
        ...
        elif self.args.dpm_solver:
            self.sampler = DPMSolverSampler(model)
        else:
            self.sampler = DDIMSampler(model)
        return model
    def sample_image(self, cand = None, batch_size=10, vanilla=False, custom_steps=None, eta=None, 
        nums =None, nplog=None, dpm=False):
        timestep_to_use = cand['timestep']
        if nums//batch_size<=1000:
            n_samples_per_class=batch_size
            classes=random.sample(range(1000),nums//batch_size)
        else:
            n_samples_per_class=nums//1000
            classes=random.sample(range(1000),1000)
        all_images = []
        ddim_eta = 0.0
        scale = 3.0
        with torch.no_grad():
            with self.model.ema_scope():
                uc = self.model.get_learned_conditioning(
                    {self.model.cond_stage_key: torch.tensor(n_samples_per_class*[1000]).to(self.model.device)}
                    )
                for class_label in classes:
                    ...
                    x_samples_ddim=x_samples_ddim.contiguous()
                    all_images.append(x_samples_ddim)
        grid = torch.cat(all_images, 0)
        return grid

    def sample_image_file_fast_process(self, cand = None, batch_size=10, vanilla=False, custom_steps=None, eta=None, 
        nums =50000, nplog=None, dpm=False,timestep_num=None):
        nums=1000
        idx=self.args.index_cate
        timestep_to_use = cand['timestep']
        if timestep_num==None:
            timestep_num=len(timestep_to_use)
        save_dir=os.path.join(self.args.logdir, str(timestep_num))
        os.makedirs(save_dir)
        n_samples_per_class=10
        classes=list(range(idx*100,(idx+1)*100))
        all_images = []
        ddim_eta = 0.0
        scale = 3.0
        cnt=0
        with torch.no_grad():
            with self.model.ema_scope():
                for jj in range(n_samples_per_class//batch_size):
                    uc = self.model.get_learned_conditioning(
                        {self.model.cond_stage_key: torch.tensor(batch_size*[1000]).to(self.model.device)} 
                        )
                    for class_label in classes:
                        ...
                        x_samples_ddim=((x_samples_ddim+1)*127.5).clamp(0,255).to(torch.uint8)
                        x_samples_ddim=x_samples_ddim.permute(0,2,3,1)
                        x_samples_ddim=x_samples_ddim.contiguous().numpy()
                        for __ in range(len(x_samples_ddim)):
                            img=Image.fromarray(x_samples_ddim[__])
                            if not img.mode == "RGB":
                                img=img.convert('RGB')
                            img.save(os.path.join(save_dir,str(cnt)+'.png'))
                            cnt=cnt+1

    def sample_image_file(self, cand = None, batch_size=10, vanilla=False, custom_steps=None, eta=None, 
        nums =50000, nplog=None, dpm=False,timestep_num=None):
        timestep_to_use = cand['timestep']
        if timestep_num==None:
            timestep_num=len(timestep_to_use)
    ...
    def sample_image_file_to_dir(self, timestep,save_dir,batch_size=25, vanilla=False, custom_steps=None, eta=None, 
        ...
        with torch.no_grad():
            with self.model.ema_scope():
                ...
                for class_label in classes:
                    ...
                    x_samples_ddim=x_samples_ddim.contiguous()
                    for __ in range(len(x_samples_ddim)):
                        img=Image.fromarray(x_samples_ddim[__].numpy(),'RGB')
                        img.save(save_dir+'/'+str(cnt)+'.png')
                        cnt+=1
                        if cnt>=nums:
                            return
...
def get_parser():
    ...
    parser.add_argument('--index_cate',type=int, default='999')
    ...
    parser.add_argument('--ref_batch',type=str,default=None)
    parser.add_argument('--timestep_group_method',type=str,default='quad')
    parser.add_argument('--on_jiang_cloud',action='store_true')
...
class EvolutionSearcher(object):
    ...
    def __init__(self, args,diffusion, time_step):
        ...
        if self.args.ptq and self.args.deterministic_retrain is False:
            if 6 in self.diffusion.weight_list and self.args.target_bitops is None:
    ...
    def get_cand_fid(self, cand=None, args=None, nums=None,index=None):
        ...
        if self.ref_stats is None:
            if 'churches' in self.args.model_ckpt:
                self.ref_stats=get_statistics(self.evaluator,'/home/hjsun/Datasets/lsun/lsun_church_50000.npz',batch_size = 25)
            elif 'beds' in self.args.model_ckpt:
                self.ref_stats=get_statistics(self.evaluator,'/home/hjsun/Datasets/lsun/lsun_bedroom_50000.npz',batch_size = 25)
            elif 'cin' in self.args.model_ckpt:
                if self.args.ref_batch != None:
                    self.ref_stats=get_statistics(self.evaluator,self.args.ref_batch,batch_size=25)
                else:
                    if self.args.on_jiang_cloud:
                        self.ref_stats=get_statistics(self.evaluator,'/home/hjsun/Datasets/ImageNet256_50000.npz',batch_size = 25)
                    else:
                        self.ref_stats=get_statistics(self.evaluator,'/home/hjsun/Datasets/ImageNet256_50000.npz',batch_size = 25)
    ...
    def get_cand_fid_file(self, cand=None, args=None, nums=None,timestep_num=None):
        ...
        if self.args.ptq:
            self.diffusion.model.model.diffusion_model.set_quant_idx_cand(cand['use_bitwidth'][0])
        if self.args.index_cate==999:
            self.diffusion.sample_image_file(cand, nums = nums,timestep_num=timestep_num)
        else:
            self.diffusion.sample_image_file_fast_process(cand, nums = nums,timestep_num=timestep_num)
        sample_time = time.time() - t1
        t1 = time.time()
    ...
    def get_fid(self,timestep=None,bit_width=None,skip_type='uniform',index=None):
        cand=dict()
        ...
        bitops_now=self.diffusion.model.model.diffusion_model.cal_bitops(bit_width)
    ...
    def get_fid_file(self,cand,timestep_num):
        ...
        if timestep is None:
            ...
            if self.args.timestep_group_method is 'quad':
                seq = (
                    np.linspace(
                ...
                seq = range(0,1000,1000//timestep_num)
            cand['timestep']=[int(s) for s in list(seq)]
            if self.args.ptq:
        ...
        if timestep is None:
            ...
            nums_image=10000
        logger.info(str(len(cand['timestep'])))
        self.get_cand_fid_file(cand,nums=nums_image,timestep_num=timestep_num)
         

    def get_fid_make_file(self,bit_width,save_dir,timestep=None,skiptype='uniform'):
        num_timesteps = self.diffusion.model.num_timesteps
        cand=dict()
        cand['use_bitwidth']=bit_width
        if timestep is None:
        ...
        nums_image=3000
        self.diffusion.model.model.diffusion_model.set_quant_idx_cand(bit_width[0])
        self.diffusion.sample_image_file_to_dir(timestep=cand['timestep'], nums = nums_image,save_dir=save_dir)
    ...
    def get_all_performance(self):
        for i in range(1):
            ...
            self.get_fid_make_file(bit_width=use_bitwidth,save_dir=args.logdir+'/uniform')
            self.get_fid_make_file(bit_width=use_bitwidth,skiptype='quad',save_dir=args.logdir+'/quad')
...
if __name__ == "__main__":
    ...
    if args.get_some_performance:
        ...
        if args.vis_dict !=None:
            searcher.continue_search()
        else:
            searcher.search()
